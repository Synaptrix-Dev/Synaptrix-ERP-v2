...
function ProjectDetails() {
    ...
    useEffect(() => {
        const fetchProject = async () => {
            try {
                setLoading(true);
                const response = await fetch(`${authURL}/root/projects/get-project-by-id?id=${id}`, {
                    headers: {
                        'x-api-key': apiKey,
                    },
                });
                    ...
                    <div className="p-6">
                        ...
                        <div className="space-y-3 mt-4">
                            <div className="flex items-center px-4 py-2 bg-amber-50 rounded-lg">
                                ...
                                <a href={`mailto:${project.clientEmail}`} className="text-blue-600 hover:underline text-sm">{project.clientEmail}</a>
                            </div>
                            <div className="flex items-center px-4 py-2 bg-amber-50 rounded-lg">
                                <svg className="w-5 h-5 mr-2 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className="p-6">
                        {project.accesibles.length > 0 ? (
                            <div className="flex items-center">
                                {project.accesibles.map((contributorId, index) => (
                                    <div
                                        ...
                                        style={{ marginLeft: index > 0 ? '-12px' : '0', zIndex: project.accesibles.length - index }}
                                    >
                                        {contributorImages[contributorId] ? (
                                            ...
                                            <div
                                                className="w-12 h-12 rounded-full bg-gradient-to-r from-green-400 to-emerald-500 flex items-center justify-center text-white font-medium text-sm border-2 border-white shadow-sm"
                                            >
                                                SS
                                            </div>
                                        )}
                            ...
                            <div className="flex items-center justify-center h-24 text-gray-500">
                                <svg className="w-6 h-6 mr-2 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
