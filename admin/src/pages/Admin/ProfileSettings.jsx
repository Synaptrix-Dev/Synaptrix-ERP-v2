...
function ProfileSettings() {
    ...
    useEffect(() => {
        const fetchAdminInfo = async () => {
            try {
                const response = await fetch(`${authURL}/auth/get-admin-info?id=${id}`, {
                    method: 'GET',
                    headers: {
                        'x-api-key': apiKey,
    ...
    const handleUpdate = async () => {
        try {
            const response = await fetch(`http://localhost:7980/api/auth/update-admin?_id=${id}`, {
                method: 'PUT',
                headers: {
                    'x-api-key': apiKey,
                    'Content-Type': 'application/json',
    ...
    return (
        <div className="min-h-screen">
            ...
            <div className="flex flex-col items-start justify-between rounded-lg border border-slate-200 bg-slate-100 my-6 p-4 mx-14">
            ...
            <div className="rounded-lg border border-slate-200 bg-white my-6 mx-14">
                        ...
                        adminData.bankDetails.map((bank, index) => (
                            <div key={index} className="grid grid-cols-1 gap-4 mb-4">
                                <div>
                                    ...
                                    <p className="text-gray-900">{bank.accName || 'N/A'}</p>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <p className="text-gray-900">{bank.accNum || 'N/A'}</p>
                                </div>
                                <div>
                                    <span className="text-sm font-medium text-gray-500">IBAN</span>
                                    <p className="text-gray-900">{bank.iban || 'N/A'}</p>
                                </div>
                            </div>
                        ))
                    ) : (
                        ...
                        <div className="space-y-2">
                            <div>
                                ...
                                <input
                                    ...
                                    class="w-full bg-white rounded border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 text-sm outline-none text-gray-700 px-3 leading-8 transition-colors duration-200 ease-in-out"
                                />
                            </div>
                            <div>
