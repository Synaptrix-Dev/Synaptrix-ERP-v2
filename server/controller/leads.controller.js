...
exports.createLead = async (req, res) => {
  try {
    const { createdBy, leadID, name, email, type, phone, location, status } =
      req.body;
    const lead = new Lead({
      createdBy,
      leadID,
...
exports.getAccessibleLeads = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      message:
        leads.length === 0
          ? "No leads found with this filter/search"
          : undefined,
    });
  } catch (error) {
...
exports.getAllLeads = async (req, res) => {
  try {
    ...
    // Query leads with sorting by leadID in descending order, then apply pagination
